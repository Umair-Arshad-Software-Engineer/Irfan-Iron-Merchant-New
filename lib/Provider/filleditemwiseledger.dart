...
class ItemsWiseLedgerProvider with ChangeNotifier {
  ...
  Future<void> fetchItemsWiseLedger(String customerId) async {
    try {
      ...
      await _db.child('filledledger').child(customerId).get();
        ...
        if (value is Map) {
          ...
          if (debit != 0.0 || credit != 0.0) {
            summaryRows.add({
              ...
              'filledNumber': tx['filledNumber'],
              'referenceNumber': tx['referenceNumber'],
              'details': credit > 0 ? 'Filled Sale' : 'Payment Received',
              'debit': debit,
              'credit': credit,
              'paymentMethod':
              tx['paymentMethod'] ?? (credit > 0 ? 'Filled' : 'Cash'),
      ...
      // 3. Build flat list: for each filled summary, fetch and inline its items
      ...
      for (var summary in summaryRows) {
        ...
        if (credit > 0) {
          final filledNumber = summary['filledNumber']?.toString() ??
              summary['referenceNumber']?.toString();
          if (filledNumber != null && filledNumber.isNotEmpty) {
            final items = await _fetchItems(filledNumber, summary);
            flat.addAll(items);
          }
        }
      }
  ...
  Future<List<Map<String, dynamic>>> _fetchItems(
      String filledNumber, Map<String, dynamic> parentSummary) async {
    try {
      ...
      final filled = _convertToMap(snapshot.snapshot.value);
      if (filled.isEmpty) return [];

      final filledRaw = filled[filled.keys.first];
      if (filledRaw is! Map) return [];

      final filledMap = _convertToMap(filledRaw);
      if (!filledMap.containsKey('items')) return [];

      return _extractItems(filledMap['items'], parentSummary);
    } catch (e) {
      print('Error fetching items for $filledNumber: $e');
      return [];
    }
  }
  ...
  List<Map<String, dynamic>> _extractItems(
    ...
    void process(Map<String, dynamic> itemMap) {
      ...
      items.add({
        'date': parent['date'],
        'filledNumber': parent['filledNumber'],
