...
//             if (invoicesData[i] != null) {
//               invoicesMap[i] = invoicesData[i];
//             }
//           }
//         }
//
...
//             if (purchasesData[i] != null) {
//               purchasesMap[i] = purchasesData[i];
//             }
//           }
//         }
//
...
//           return [
//             pw.Text('Item Transactions Report', style: pw.TextStyle(fontSize: 24, fontWeight: pw.FontWeight.bold)),
...
//       ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to share PDF')));
//     }
//   }
//
...
//                                 ? (languageProvider.isEnglish ? 'Select Start Date' : 'شروع کی تاریخ منتخب کریں')
...
//                                 ? (languageProvider.isEnglish ? 'Select End Date' : 'اختتام کی تاریخ منتخب کریں')
...
//                         labelText: languageProvider.isEnglish ? 'Filter by Item' : 'آئٹم کے لحاظ سے فلٹر کریں',
//                         border: OutlineInputBorder(),
//                       ),
//                       items: [
//                         DropdownMenuItem(value: null, child: Text(languageProvider.isEnglish ? 'All Items' : 'تمام آئٹمز')),
//                         ..._itemNames.map((item) => DropdownMenuItem(value: item, child: Text(item))).toList(),
//                       ],
//                       onChanged: (value) {
//                         setState(() {
//                           _selectedItem = value;
...
//               children: [
//                 _buildSummaryCard(
//                   languageProvider.isEnglish ? 'Total Sales' : 'کل فروخت',
//                   _filteredTransactions.where((t) => t['type'].toString().contains('Sale')).fold(0.0, (sum, t) => sum + (t['total'] ?? 0.0)),
//                   Colors.green,
//                 ),
//                 SizedBox(width: 8),
//                 _buildSummaryCard(
//                   languageProvider.isEnglish ? 'Total Purchases' : 'کل خریداری',
//                   _filteredTransactions.where((t) => t['type'] == 'Purchase').fold(0.0, (sum, t) => sum + (t['total'] ?? 0.0)),
//                   Colors.blue,
//                 ),
//               ],
//             ),
...
//                   ? Center(child: Text(languageProvider.isEnglish ? 'No transactions found' : 'کوئی لین دین نہیں ملا', style: TextStyle(fontSize: 18)))
...
//                       DataColumn(label: Text(languageProvider.isEnglish ? 'Doc No.' : 'دستاویز نمبر')),
//                       DataColumn(label: Text(languageProvider.isEnglish ? 'Customer/Vendor' : 'کسٹمر/فروش')),
//                       DataColumn(label: Text(languageProvider.isEnglish ? 'Item' : 'آئٹم')),
//                       DataColumn(label: Text(languageProvider.isEnglish ? 'Qty' : 'مقدار'), numeric: true),
//                       if (_filteredTransactions.any((t) => t['weight'] != null && t['weight'] > 0))
//                         DataColumn(label: Text(languageProvider.isEnglish ? 'Weight' : 'وزن'), numeric: true),
//                       DataColumn(label: Text(languageProvider.isEnglish ? 'Rate' : 'ریٹ'), numeric: true),
//                       DataColumn(label: Text(languageProvider.isEnglish ? 'Total' : 'کل'), numeric: true),
//                     ],
//                     rows: _filteredTransactions.map((transaction) {
//                       return DataRow(cells: [
//                         DataCell(Text(DateFormat('yyyy-MM-dd').format(transaction['date']))),
//                         DataCell(Text(transaction['type'])),
//                         DataCell(Text(transaction['type'] == 'Invoice Sale'
//                             ? transaction['invoiceNumber'].toString()
//                             : transaction['type'] == 'Filled Sale'
//                             ? transaction['filledNumber'].toString()
//                             : '-')),
//                         DataCell(Text(transaction['type'] == 'Purchase'
                      ...
                      child: Row(
                        children: [
                          _buildSummaryCard(
                            ...
                            _filteredTransactions.where((t) => t['type'].toString().contains('Sale'))
                          ...
                          _buildSummaryCard(
                            ...
                            _filteredTransactions.where((t) => t['type'] == 'Purchase')
                                .fold(0.0, (sum, t) => sum + (t['total'] ?? 0.0)),
                            Colors.blue,
                          ),
                        ],
